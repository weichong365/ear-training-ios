...
import { DEFAULT_AUDIO_VOLUME, normalizeAudioVolume } from '@/core/audio-settings';
...
import { Btn, hitSlopFor, hitSlopVertical } from '@/constants/button-tokens';
import { Brand, Radius, Shadows, TypeScale } from '@/constants/theme';
...
const MODE_NAMES: Record<PracticeMode, string> = {
  single: '单音听记', group: '音组听记', interval: '音程听记', connection: '和声音程连接', chord: '和弦听记', chordQuality: '和弦性质', chordPitch: '和弦音高', rhythm: '节奏听记', melody: '旋律听记', adaptive: '智能强化',
};
...
export default function PracticeScreen() {
  ...
  const questionSnapshots = useRef<(PracticeQuestionSnapshot | undefined)[]>([]);
  ...
  const volumeRow = <View style={[styles.volumeRow, compactPitchMode && styles.compactVolumeRow]}><Text style={styles.volumeLabel}>音量</Text><Pressable accessibilityRole="button" accessibilityLabel="降低音量" hitSlop={hitSlopFor(Btn.practice.choicePill.minHeight)} onPress={() => changeVolume(volume - 10)} style={styles.volumeStep}><AppIcon name="minus" size={13} /></Pressable><View accessibilityRole="progressbar" accessibilityValue={{ min: 0, max: 100, now: volume }} style={styles.volumeTrack}><View style={[styles.volumeFill, { width: `${volume}%` }]} /></View><Pressable accessibilityRole="button" accessibilityLabel="提高音量" hitSlop={hitSlopFor(Btn.practice.choicePill.minHeight)} onPress={() => changeVolume(volume + 10)} style={styles.volumeStep}><AppIcon name="plus" size={13} /></Pressable><Text style={styles.volumeValue}>{volume}%</Text></View>;
  ...
  useEffect(() => {
    ...
    // restorePracticeSnapshot intentionally reads the state captured with this session.
    // Re-running this effect when the helper identity changes would overwrite current progress.
  ...
  useEffect(() => {
    ...
    // capturePracticeSnapshot intentionally snapshots the dependencies listed below.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const keyboardUnavailable = phase !== 'feedback' || playing || preparing;
      ...
      {phase === 'finished' ? <View style={styles.resultCard}>
        ...
        <View style={[styles.card, compactPitchMode && styles.compactCard]}>
          <Pressable accessibilityRole="button" accessibilityLabel={playing || preparing ? '音频播放中' : phase === 'feedback' ? '回放正确答案' : phase === 'ready' ? '播放题目' : '再听一遍'} accessibilityState={{ disabled: playing || preparing || (phase !== 'feedback' && playCount >= maxPlays), busy: playing || preparing }} onPress={() => void play()} disabled={playing || preparing || (phase !== 'feedback' && playCount >= maxPlays)} style={({ pressed }) => [styles.playButton, compactPitchMode && styles.compactPlayButton, (playing || preparing) && styles.playing, pressed && styles.pressed]}>
            <View style={styles.playIcon}><AppIcon name={playing || preparing ? 'pause' : 'play'} size={16} color={Brand.textOnAccent} /></View><View style={styles.playCopy}><Text style={styles.playTitle}>{playing ? '播放中…' : phase === 'feedback' ? '回放答案' : phase === 'ready' ? '播放题目' : '再听一遍'}</Text></View><Text style={styles.replay}>{phase === 'feedback' ? '不限次数' : `剩余 ${Math.max(0, maxPlays - playCount)} 次`}</Text>
          </Pressable>
          {volumeRow}
        ...
        <View style={[styles.keyboardCard, phase === 'feedback' && styles.keyboardOpen]}>
          ...
          <View style={styles.keyboardStage}><View aria-hidden={keyboardUnavailable} accessibilityElementsHidden={keyboardUnavailable} importantForAccessibility={keyboardUnavailable ? 'no-hide-descendants' : 'auto'}><PianoKeyboard startMidi={55} endMidi={81} disabled={keyboardUnavailable} highlights={highlights} volume={volume} onKeyPress={phase === 'feedback' ? reviewPianoKey : undefined} /></View>{phase !== 'feedback' && <View accessibilityRole="text" accessibilityLabel="复盘钢琴待解锁，提交答案后解锁" style={styles.keyboardLock}><View style={styles.lockIcon}><View style={styles.lockShackle} /><View style={styles.lockBody}><View style={styles.lockHole} /></View></View></View>}</View>
        </View>
      </>}
    </ScrollView>

    {phase !== 'finished' && (phase === 'feedback' ? <View style={[styles.bottomActions, { paddingBottom: Math.max(10, insets.bottom) }]}>
      {index > 0 && <Pressable accessibilityRole="button" accessibilityLabel="上一题" accessibilityState={{ disabled: playing || preparing }} disabled={playing || preparing} hitSlop={hitSlopVertical(Btn.practice.feedback.minHeight)} onPress={previous} style={styles.secondarySmall}><Text style={styles.secondaryText}>上一题</Text></Pressable>}
      <Pressable accessibilityRole="button" accessibilityLabel="回放答案" accessibilityState={{ disabled: playing || preparing }} disabled={playing || preparing} hitSlop={hitSlopVertical(Btn.practice.feedback.minHeight)} onPress={() => void play()} style={styles.secondarySmall}><Text style={styles.secondaryText}>回放答案</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel={index + 1 >= questions.length ? '查看结果' : '下一题'} accessibilityState={{ disabled: playing || preparing }} disabled={playing || preparing} hitSlop={hitSlopVertical(Btn.practice.feedback.minHeight)} onPress={next} style={styles.primarySmall}><Text style={styles.primaryText}>{index + 1 >= questions.length ? '查看结果' : '下一题'}</Text></Pressable>
    </View> : <View style={[styles.bottomBar, { paddingBottom: Math.max(10, insets.bottom) }]}>
      <Pressable accessibilityRole="button" accessibilityState={{ disabled: !complete || phase !== 'answering' }} disabled={!complete || phase !== 'answering'} hitSlop={hitSlopFor(Btn.practice.submit.minHeight)} onPress={() => void submit()} style={[styles.submit, (!complete || phase !== 'answering') && styles.submitDisabled]}><Text style={styles.primaryText}>{phase === 'ready' ? '先播放题目' : complete ? '提交答案并解锁键盘' : '完成谱面后提交'}</Text></Pressable><Text style={styles.submitTip}>{phase === 'ready' ? '播放后即可在谱面上作答' : '提交后显示正确谱面，并解锁复盘钢琴'}</Text>
    </View>)}
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  card: { padding: 13, borderRadius: Radius.card, backgroundColor: Brand.ivory, borderWidth: 1, borderColor: Brand.border }, playButton: { ...Btn.practice.playButton, flexDirection: 'row', alignItems: 'center', backgroundColor: Brand.forest }, playing: { backgroundColor: Brand.success }, playIcon: { ...Btn.practice.playIconWrap, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,255,255,.15)' }, playCopy: { flex: 1 }, playTitle: { color: Brand.textOnAccent, fontSize: 13.5, fontWeight: '700' }, replay: { paddingLeft: 7, color: 'rgba(255,255,255,.9)', fontSize: 12, fontWeight: '700' }, pressed: { opacity: 0.78 }, volumeRow: { marginTop: 3, flexDirection: 'row', alignItems: 'center', gap: 7 }, volumeLabel: { color: '#8A90A2', fontSize: Btn.practice.volumeRow.fontSize }, volumeStep: { ...Btn.practice.choicePill, alignItems: 'center', justifyContent: 'center', backgroundColor: Brand.forestSoft }, volumeTrack: { flex: 1, height: 5, overflow: 'hidden', borderRadius: 5, backgroundColor: Brand.divider }, volumeFill: { height: '100%', backgroundColor: Brand.forest }, volumeValue: { width: 31, color: '#8A90A2', fontSize: Btn.practice.volumeRow.fontSize, textAlign: 'right', fontVariant: ['tabular-nums'] }, error: { marginTop: 7, color: Brand.danger, fontSize: TypeScale.footnote, lineHeight: 18 },
  // 紧凑模式是小程序没有的 iOS 优化（键盘弹出时压缩），在 1:1 播放入口基础上再收 5pt 内边距。
  compactCard: { padding: 8 }, compactPlayButton: { paddingVertical: 5, paddingHorizontal: 7 }, compactVolumeRow: { marginTop: 4 },
  ...
  qualityBlock: { gap: 8 }, qualityLabel: { color: Brand.muted, fontSize: TypeScale.footnote, fontWeight: '700' }, qualityGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 }, qualityOption: { ...Btn.practice.choicePill, alignItems: 'center', justifyContent: 'center', borderColor: Brand.border, backgroundColor: Brand.ivory }, qualityActive: { borderColor: Brand.forest, backgroundColor: Brand.forest }, qualityText: { color: Brand.ink, fontSize: Btn.practice.choicePill.fontSize, fontWeight: '700' }, qualityTextActive: { color: Brand.textOnAccent },
  keyboardCard: { padding: 13, borderRadius: Radius.card, borderWidth: 1, borderColor: Brand.border, backgroundColor: Brand.successSoft }, keyboardOpen: { borderColor: '#2e8b6f' }, keyboardHead: { marginBottom: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, keyboardTitle: { color: Brand.ink, fontSize: TypeScale.subheadline, fontWeight: '900' }, keyboardSub: { marginTop: 3, color: Brand.muted, fontSize: TypeScale.caption }, keyboardState: { paddingHorizontal: 9, paddingVertical: 5, borderRadius: 8, color: Brand.warning, backgroundColor: Brand.warningSoft, fontSize: 11, fontWeight: '700' }, keyboardStateOpen: { color: Brand.forest, backgroundColor: Brand.forestSoft }, keyboardStage: { position: 'relative', overflow: 'hidden', borderRadius: Radius.control }, keyboardLock: { position: 'absolute', zIndex: 8, left: 0, right: 0, top: 0, height: 150, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(31,35,52,.68)' }, lockIcon: { width: 52, height: 52, alignItems: 'center', justifyContent: 'center', borderRadius: 16, backgroundColor: 'rgba(255,255,255,.14)' }, lockShackle: { position: 'absolute', top: 9, width: 23, height: 19, borderWidth: 2, borderBottomWidth: 0, borderColor: Brand.textOnAccent, borderTopLeftRadius: 12, borderTopRightRadius: 12 }, lockBody: { position: 'absolute', top: 25, width: 31, height: 23, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: Brand.textOnAccent, borderRadius: 5 }, lockHole: { width: 10, height: 3, borderRadius: 3, backgroundColor: Brand.textOnAccent },
  bottomBar: { position: 'absolute', left: 10, right: 10, bottom: 0, padding: 10, alignItems: 'center' }, submit: { width: '100%', ...Btn.practice.submit, alignItems: 'center', justifyContent: 'center', backgroundColor: Brand.forest }, submitDisabled: { backgroundColor: Brand.disabled }, submitTip: { marginTop: 7, color: Brand.muted, fontSize: TypeScale.caption, lineHeight: 16 }, bottomActions: { position: 'absolute', left: 10, right: 10, bottom: 0, padding: 10, flexDirection: 'row', gap: 8 }, primarySmall: { flex: 1, ...Btn.practice.feedback, alignItems: 'center', justifyContent: 'center', backgroundColor: Brand.forest, ...Shadows.raised }, secondarySmall: { flex: 1, ...Btn.practice.feedback, alignItems: 'center', justifyContent: 'center', borderColor: Brand.border, backgroundColor: Brand.ivory }, primaryText: { color: Brand.textOnAccent, fontSize: Btn.practice.feedback.fontSize, fontWeight: '900' }, secondaryText: { color: Brand.forest, fontSize: Btn.practice.feedback.fontSize, fontWeight: '900' },
  resultCard: { marginTop: 24, padding: 28, alignItems: 'center', borderRadius: Radius.hero, backgroundColor: Brand.ivory, borderWidth: 1, borderColor: Brand.border }, resultTitle: { marginTop: 14, color: Brand.ink, fontSize: TypeScale.title3, fontWeight: '900' }, resultScore: { marginTop: 6, color: Brand.forest, fontSize: 58, fontWeight: '900', fontVariant: ['tabular-nums'] }, resultDesc: { color: Brand.ink, fontSize: TypeScale.subheadline, fontWeight: '800' }, resultSub: { marginTop: 8, color: Brand.muted, fontSize: TypeScale.footnote }, primary: { width: '100%', ...Btn.global.primary, marginTop: 24, alignItems: 'center', justifyContent: 'center', backgroundColor: Brand.forest }, secondary: { width: '100%', ...Btn.global.plain, marginTop: 10, alignItems: 'center', justifyContent: 'center', borderColor: Brand.border, backgroundColor: Brand.ivory },
});
